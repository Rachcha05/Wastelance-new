  ...
  try {
    ...
    User.findOne({ email: req.body.email }).exec(async (err, user) => {
      if (user){
        ...
        User.findOne({ nic: req.body.nic }).exec(async (err, user) => {
          if (user){
            exUser = user;
            return res.status(202).json({
              errormsg: "User already with given NIC!",
            });
          }
        });
              ...
              if (user) {
                ...
                if(transporter){
                  transporter.sendMail({
                    to: user.email,
                    from: "rakka@nipunamu.com",
                    subject: "Signup Successfull - CleanTech",
                    html: `<html lang="en">
                    <head>
                      <meta charset="UTF-8">
                                        ...
                                        <tr>
                                          ...
                                          <td width="330" style="width: 33%;" align="right">
                                            <div style="text-align: center; max-width: 150px; width: 100%;">
                                              <span>&nbsp;</span>
                                              <a href='https://web.facebook.com/Clean-tech-108681087536234/' target="_blank" border="0" style="border: none; outline: none; text-decoration: none; line-height: 60px; color: #ffffff; font-family: Verdana, Geneva, sans-serif;  -webkit-text-size-adjust:none">
    ...
    User.findOne({ email: req.body.email }).then((user) => {
      ...
      exports.sendEmail = (req, res) => {
        ...
        const Obj = {
          ...
          subject: "Password Reset - CleanTech",
